...
#include "vncdisplaykeymap.hpp"
#include "vncdisplayenums.hpp"
#include "vnccairoframebuffer.hpp"
...
static const GOptionEntry gtk_vnc_args[] =
    {
        { "gtk-vnc-debug", 0, G_OPTION_FLAG_NO_ARG, G_OPTION_ARG_CALLBACK,
          (gpointer)vnc_debug_option_arg, N_("Enables debug output"), 0 },
                          ...
                          GParamSpec   *pspec)
{
    ...
    switch (prop_id)
        ...
        case PROP_FORCE_SIZE:
            ...
            break;
        case PROP_DEPTH:
            vnc_display_set_depth (vnc, (VncDisplayDepthColor)g_value_get_enum (value));
            break;
        case PROP_GRAB_KEYS:
            vnc_display_set_grab_keys(vnc, (VncGrabSequence *)g_value_get_boxed(value));
            break;
        default:
            G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
            break;
...
static void do_keyboard_grab_all(GdkWindow *window)
{
    ...
    GList *tmp = devices;
    while (tmp) {
        GdkDevice *dev = (GdkDevice *)tmp->data;
                              ...
                              gpointer opaque)
{
    ...
    do_framebuffer_init((VncDisplay *)opaque, remoteFormat, width, height, FALSE);
                                    ...
                                    gpointer opaque)
{
    ...
    do_framebuffer_init((VncDisplay *)opaque, remoteFormat, width, height, TRUE);
...
static gboolean check_pixbuf_support(const char *name)
{
    ...
    for (i = list; i; i = i->next) {
        GdkPixbufFormat *fmt = (GdkPixbufFormat *)i->data;
...
{
    g_return_val_if_fail (VNC_IS_DISPLAY (obj), (VncDisplayDepthColor) 0);
