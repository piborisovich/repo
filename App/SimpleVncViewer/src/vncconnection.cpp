...
#include "vncconnection.hpp"
#include "vncconnectionenums.hpp"
#include "vncmarshal.hpp"
#include "vncutil.hpp"
...
static gboolean g_io_wait_helper(GSocket *sock G_GNUC_UNUSED,
                                 ...
                                 gpointer data)
{
    struct coroutine *to = (coroutine *)data;
...
static GIOCondition g_io_wait(GSocket *sock, GIOCondition cond)
{
    ...
    ret = (GIOCondition *)coroutine_yield(NULL);
    g_source_unref(src);
    return *ret;
}
                                        ...
                                        GParamSpec *pspec)
{
    ...
    switch (prop_id) {
    case PROP_FRAMEBUFFER:
        vnc_connection_set_framebuffer(conn, (VncFramebuffer *)g_value_get_object(value));
        break;
...
{
    struct signal_data *data = (signal_data *)opaque;
...
static ssize_t vnc_connection_tls_push(gnutls_transport_ptr_t transport,
                                       const void *data,
                                       size_t len) {
    VncConnection *conn = (VncConnection *)transport;
    VncConnectionPrivate *priv = conn->priv;
    int ret;
    GError *error = NULL;
    ...
    ret = g_socket_send(priv->sock, (const gchar *)data, len, NULL, &error);
    if (ret < 0) {
        if (error) {
            if (error->code == G_IO_ERROR_WOULD_BLOCK)
...
static ssize_t vnc_connection_tls_pull(gnutls_transport_ptr_t transport,
    ...
    ret = g_socket_receive(priv->sock, (gchar *)data, len, NULL, &error);
    if (ret < 0) {
        if (error) {
            if (error->code == G_IO_ERROR_WOULD_BLOCK)
                errno = EAGAIN; /* For gnutls compat */
...
static void vnc_connection_buffered_write(VncConnection *conn, const void *data, size_t size)
{
    ...
    if (left < size) {
        ...
        priv->xmit_buffer = (char *)g_realloc(priv->xmit_buffer, priv->xmit_buffer_capacity);
    }
                                      ...
                                      guint16 width, guint16 height)
{
    ...
    */
    if (vnc_framebuffer_perfect_format_match(priv->fb)) {
        ...
        dst = (guint8 *)g_malloc(width * (priv->fmt.bits_per_pixel / 8));
        for (i = 0; i < height; i++) {
                                       ...
                                       guint16 width, guint16 height)
{
    ...
    zlib_data = (guint8 *)g_malloc(length);
                                                 ...
                                                 guint16 width, guint16 height)
{
    ...
    bpp = vnc_connection_pixel_size(conn);
    last_row = (guint8 *)g_malloc(width * bpp);
    row = (guint8 *)g_malloc(width * bpp);
                                             ...
                                             guint8 *data, size_t length)
{
    ...
    p = (GdkPixbuf *)g_object_ref(gdk_pixbuf_loader_get_pixbuf(loader));
                                        ...
                                        guint16 width, guint16 height)
{
    ...
    if (ccontrol <= 7) {
        ...
        if (data_size >= 12) {
            ...
            zlib_data = (guint8 *)g_malloc(zlib_length);
        ...
        jpeg_data = (guint8 *)g_malloc(length);
...
static void vnc_connection_rich_cursor(VncConnection *conn, int x, int y, int width, int height)
{
    ...
    if (width && height) {
        ...
        image = (guint8 *)g_malloc(imagelen);
        mask = (guint8 *)g_malloc(masklen);
        pixbuf = (guint8 *)g_malloc(width * height * 4); /* RGB-A 8bit */
...
static void vnc_connection_xcursor(VncConnection *conn, int x, int y, int width, int height)
{
    ...
    if (width && height) {
        ...
        rowlen = ((width + 7)/8);
        data = (guint8 *)g_malloc(rowlen*height);
        mask = (guint8 *)g_malloc(rowlen*height);
        pixbuf = (guint8 *)g_malloc(width * height * 4); /* RGB-A 8bit */
...
{
    VncConnection *conn = (VncConnection *)opaque;
    VncConnectionPrivate *priv = conn->priv;
...
{
    struct audio_action_data *data = (audio_action_data *)opaque;
    VncConnectionPrivate *priv = data->conn->priv;

    VNC_DEBUG("Audio action main context %u", data->action);

    switch (data->action) {
    case audio_action_data::VNC_AUDIO_PLAYBACK_STOP:
        vnc_audio_playback_stop(priv->audio);
        break;
    case audio_action_data::VNC_AUDIO_PLAYBACK_START:
        vnc_audio_playback_start(priv->audio, &priv->audio_format);
        break;
    case audio_action_data::VNC_AUDIO_PLAYBACK_DATA:
        vnc_audio_playback_data(priv->audio, priv->audio_sample);
        break;
    default:
        g_warn_if_reached();
                                        ...
                                        int action)
{
    struct audio_action_data data = {
        conn,
        coroutine_self(),
        (audio_action_data::vnc_audio_playback)action
    };
        ...
        case VNC_CONNECTION_SERVER_MESSAGE_QEMU_AUDIO: {
            ...
            case VNC_CONNECTION_SERVER_MESSAGE_QEMU_AUDIO_DATA:
                ...
                if (priv->audio_sample &&
                    ...
                    vnc_connection_audio_action(conn, audio_action_data::VNC_AUDIO_PLAYBACK_DATA);
                    vnc_audio_sample_free(priv->audio_sample);
                    priv->audio_sample = NULL;
                }
                if (!priv->audio_sample) {
            ...
            case VNC_CONNECTION_SERVER_MESSAGE_QEMU_AUDIO_START:
                if (priv->audio)
                    vnc_connection_audio_action(conn, audio_action_data::VNC_AUDIO_PLAYBACK_START);
                else
            ...
            case VNC_CONNECTION_SERVER_MESSAGE_QEMU_AUDIO_STOP:
                if (priv->audio) {
                    ...
                    vnc_connection_audio_action(conn, audio_action_data::VNC_AUDIO_PLAYBACK_STOP);
                } else {
                    vnc_connection_set_error(conn, "%s", "No audio sink configured");
                }
                break;
...
{
    VncConnection *conn = (VncConnection *)data;
    VncConnectionPrivate *priv = conn->priv;
...
static gboolean vnc_connection_perform_auth_ard(VncConnection *conn)
{
    ...
    mod = (guchar *)malloc(keylen);
    if (mod == NULL) {
        VNC_DEBUG("malloc failed\n");
        return FALSE;
    }
    resp = (guchar *)malloc(keylen);
    if (resp == NULL) {
        free(mod);
        VNC_DEBUG("malloc failed\n");
        return FALSE;
    }
    pub = (guchar *)malloc(keylen);
    if (pub == NULL) {
        free(resp);
        free(mod);
        VNC_DEBUG("malloc failed\n");
...
{
    struct wait_queue *wait = (wait_queue *)data;
    ...
    if (!g_socket_connect(sock, sockaddr, NULL, error)) {
        if (*error && (*error)->code == G_IO_ERROR_PENDING) {
            ...
            *error = NULL;
            ...
            if (!g_io_wait_interruptable(wait, sock, (GIOCondition)(G_IO_OUT | G_IO_ERR | G_IO_HUP))) {
...
gboolean vnc_connection_open_addr(VncConnection *conn, GSocketAddress *addr, const char *hostname)
{
    ...
    priv->fd = -1;
    priv->addr = (GSocketAddress*)g_object_ref(addr);
